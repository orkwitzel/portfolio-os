...
import type { NormalGeometry, WindowRecord } from '../desktop/sessionTypes'
import { useWindowManager, type WindowManagerApi } from '../desktop/windowManagerContext'
...
export function WindowFrame({ window: win }: { window: WindowRecord }) {
  ...
  if (win.geometry.mode === 'minimized') return null

  const rect: NormalGeometry =
    win.geometry.mode === 'normal' ? win.geometry.geometry : win.geometry.frame
  ...
  return (
    ...
    >
      <div
        className={`${styles.titleBar} ${focused ? styles.titleActive : styles.titleInactive}`}
